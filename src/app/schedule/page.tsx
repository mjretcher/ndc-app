...
import { PrintButton } from "@/app/(app)/calendar/print/PrintButton";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Practice Calendar | Napoleon Diving Club",
  description: "Download or print Napoleon Diving Club's monthly practice calendar.",
};

export default async function PublicSchedulePage({ searchParams }: { searchParams: Promise<{ y?: string; m?: string }> }) {
  ...
  const practices = club
    ? await db.query.practices.findMany({
        where: and(
          eq(tables.practices.clubId, club.id),
  ...
  return (
    <div className="bg-paper text-ink min-h-dvh">
      ...
      <div className="mx-auto max-w-5xl px-5 md:px-10 py-6 space-y-4">
        <header className="no-print flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            <Link href={`/schedule?y=${prev.y}&m=${prev.m}`} className="btn btn-secondary" aria-label="Previous month">←</Link>
            <Link href="/schedule" className="btn btn-secondary">This month</Link>
            <Link href={`/schedule?y=${next.y}&m=${next.m}`} className="btn btn-secondary" aria-label="Next month">→</Link>
            <PrintButton />
          </div>
        </header>
        ...
        <div className="hidden print:flex items-center gap-4 border-b border-line pb-3 mb-1">
          ...
          {primaryFacility && (
            <p className="ml-auto text-xs text-mute self-end">Practices at {primaryFacility} unless noted</p>
          )}
        </div>
                          ...
                          if (p.status === "canceled") {
                            return (
                              <div key={p.id} className="rounded px-1.5 py-1 text-[0.72rem] print:text-[9px] bg-line text-mute leading-snug">
                                No practice{p.publicDescription ? ` — ${p.publicDescription}` : ""}
                              </div>
                            );
                          }
